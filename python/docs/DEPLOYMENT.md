...
3. **Copy files**
   ```bash
   cp -r src/* ~/.config/mcp/devflow-monitor/
   cp pyproject.toml poetry.lock ~/.config/mcp/devflow-monitor/
   ...
   ```json
   {
     "mcpServers": {
       "devflow-monitor": {
         ...
         "cwd": "~/.config/mcp/devflow-monitor",
         "env": {
...
```json
{
  "mcpServers": {
    "devflow-monitor": {
      "command": "docker",
      "args": ["exec", "-i", "devflow-monitor", "python", "-m", "devflow_monitor"]
    }
  }
}
```
...
- Check Python version: `python -v` (must be 20+)
...
**Local:**
```bash
DEBUG=true LOG_LEVEL=debug python server/index.js
```

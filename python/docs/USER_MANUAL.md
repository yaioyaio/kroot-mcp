...
```bash
# 대시보드 실행 (CLI 모드)
poetry run python -m devflow_monitor.dashboard

# 대시보드 실행 (TUI 모드)
poetry run python -m devflow_monitor.dashboard:tui

# 메트릭 리포트 생성
poetry run python -m devflow_monitor.reports

# 현재 상태 확인
poetry run python -c "from devflow_monitor.server.main import DevFlowMonitorServer; print(DevFlowMonitorServer().get_status())"
```

## MCP 도구 활용

DevFlow Monitor는 88개의 MCP 도구를 제공합니다. 주요 도구들의 사용법을 소개합니다.
...
```bash
# 연결 상태 확인
poetry run python -m devflow_monitor.diagnose

# 설정 파일 검증
python -e "console.log(JSON.parse(require('fs').readFileSync('claude-desktop-config.json', 'utf8')))"
```
...
```bash
# 성능 진단
poetry run python -m devflow_monitor.performance.analyze

# 캐시 정리
poetry run python -c "from devflow_monitor.performance import clear_cache; clear_cache()"
```
...
```bash
# 파일 모니터링 상태 확인
poetry run python -m devflow_monitor.monitors.status

# 디버그 모드로 실행
DEBUG=file:* poetry run python -m devflow_monitor
```
...
```bash
# 상세 로그
LOG_LEVEL=debug poetry run python -m devflow_monitor

# 오류만 표시
LOG_LEVEL=error poetry run python -m devflow_monitor
```
...
# Git 모니터링 디버그
DEBUG=git:* poetry run python -m devflow_monitor

# 이벤트 시스템 디버그
DEBUG=events:* poetry run python -m devflow_monitor
```
...
A: 모든 유형의 프로젝트를 지원합니다. 특히 Python, JavaScript, Python, Java, Go 등의 언어로 작성된 프로젝트에 최적화되어 있습니다.
...
**참고**: 이 매뉴얼은 DevFlow Monitor MCP (Python) 기준으로 작성되었습니다. 최신 정보는 [GitHub 저장소](https://github.com/yaioyaio/kroot-mcp)를 확인하세요.

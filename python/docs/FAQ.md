...
- **언어**: Python, JavaScript, TypeScript, Java, Go, Rust, C#, PHP 등
...
# 오래된 데이터 정리
poetry run python -m devflow_monitor.storage.cleanup
```
...
3. **메모리 제한 설정**:
   ```bash
   python --max-old-space-size=2048 src/server/index.js
   ```
...
3. **CLI 대시보드 사용**:
   ```bash
   poetry run python -m devflow_monitor.dashboard  # TUI 대신 CLI 사용
   ```

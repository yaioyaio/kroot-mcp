...
```bash
# 초기화 테스트
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05"}}' | node dist/server/index.js

# 도구 목록 확인
(echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05"}}'; 
 sleep 0.5; 
 echo '{"jsonrpc":"2.0","id":2,"method":"tools/list"}') | node dist/server/index.js
```
...
```json
{
  ...
  "configurations": [
    ...
    },
    {
      ...
      "runtimeArgs": ["node", "${workspaceFolder}/dist/server/index.js"],
      "console": "integratedTerminal"
    }
  ]
}
...
```json
{
  "scripts": {
    ...
    "test:mcp:watch": "nodemon --watch src --ext ts --exec 'npm run build && npm run test:mcp'",
    "dev:mcp": "npm run build && mcp-inspector node dist/server/index.js",
    "debug:mcp": "node --inspect dist/server/index.js"
  }
}
```
...
1. **로그 레벨 조정**: `DEBUG=* node dist/server/index.js`

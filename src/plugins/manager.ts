...
export class PluginManager extends EventEmitter {
  ...
  private setupEventHandlers(): void {
    ...
    this.loader.on('plugin.error', (event) => {
      ...
      console.error(`[PluginManager] Plugin error: ${event._pluginId}`, event.error);
    });
   ...
   * 플러그인 제거
   */
  async uninstallPlugin(_pluginId: string): Promise<boolean> {
    try {
      // console.log(`[PluginManager] Uninstalling plugin: ${_pluginId}`);
      ...
      // 레지스트리에서 제거
      if (this.registry) {
        await this.registry.uninstallPlugin(_pluginId);
      }

      this.emit('plugin.uninstalled', { _pluginId });
      // console.log(`[PluginManager] Plugin uninstalled: ${_pluginId}`);

      return true;
    } catch (error) {
      console.error(`[PluginManager] Failed to uninstall plugin ${_pluginId}:`, error);
      this.emit('plugin.uninstall.failed', { _pluginId, error });
      return false;
    }
  }
  ...
  async loadPlugin(_pluginId: string): Promise<boolean> {
    return this.loader.loadPlugin(_pluginId);
  }
  ...
  async unloadPlugin(_pluginId: string): Promise<boolean> {
    return this.loader.unloadPlugin(_pluginId);
  }
  ...
  async activatePlugin(_pluginId: string): Promise<boolean> {
    return this.loader.activatePlugin(_pluginId);
  }
  ...
  async deactivatePlugin(_pluginId: string): Promise<boolean> {
    return this.loader.deactivatePlugin(_pluginId);
  }

  /**
   * 플러그인 재시작
   */
  async restartPlugin(_pluginId: string): Promise<boolean> {
    try {
      // console.log(`[PluginManager] Restarting plugin: ${_pluginId}`);

      await this.deactivatePlugin(_pluginId);
      await new Promise(resolve => setTimeout(resolve, 1000)); // 1초 대기
      const success = await this.activatePlugin(_pluginId);

      if (success) {
        this.emit('plugin.restarted', { _pluginId });
        // console.log(`[PluginManager] Plugin restarted: ${_pluginId}`);
      }

      return success;
    } catch (error) {
      console.error(`[PluginManager] Failed to restart plugin ${_pluginId}:`, error);
      this.emit('plugin.restart.failed', { _pluginId, error });
      return false;
    }
  }
  ...
  getPluginStatus(_pluginId: string): PluginStatus | null {
    return this.loader.getPluginStatus(_pluginId);
  }

  /**
   * 플러그인 정보 조회 (상세)
   */
  getPluginInfo(_pluginId: string): any {
    const plugins = this.getPlugins();
    const plugin = plugins.find(p => p.id === _pluginId);
    ...
    return {
      ...plugin,
      status: this.getPluginStatus(_pluginId),
      metrics: this.getPluginMetrics(_pluginId),
  ...
  async checkPluginHealth(_pluginId: string): Promise<PluginHealthStatus | null> {
    return this.loader.checkPluginHealth(_pluginId);
  }
  ...
  private startHealthCheck(): void {
    ...
    this.healthCheckTimer = setInterval(async () => {
      try {
        ...
        for (const [_pluginId, health] of Object.entries(healthResults)) {
          if (health && health.status === 'error') {
            this.emit('plugin.health.critical', { _pluginId, health });
          }
        }
   ...
   * 플러그인 업데이트 체크
   */
  async checkForUpdates(): Promise<Array<{ __pluginId: string; currentVersion: string; latestVersion: string }>> {
    if (!this.registry) {
      return [];
    }
   ...
   * 플러그인 업데이트
   */
  async updatePlugin(_pluginId: string, version?: string): Promise<boolean> {
    if (!this.registry) {
      throw new Error('Plugin registry not configured');
    }

    try {
      const plugin = this.getPlugins().find(p => p.id === _pluginId);
      if (!plugin) {
        throw new Error(`Plugin not found: ${_pluginId}`);
      }

      // console.log(`[PluginManager] Updating plugin: ${_pluginId}`);
      ...
      if (success) {
        ...
        this.emit('plugin.updated', { _pluginId, version });
        // console.log(`[PluginManager] Plugin updated: ${_pluginId}`);
      }

      return success;
    } catch (error) {
      console.error(`[PluginManager] Failed to update plugin ${_pluginId}:`, error);
      this.emit('plugin.update.failed', { _pluginId, error });
      return false;
    }
  }
}
...
export class MetricsRepository extends BaseRepository<MetricsRecord> {
   ...
   */
  async findByTypeAndName(metricType: string, metricName: string, options?: QueryOptions): Promise<MetricsRecord[]> {
   ...
   */
  async findByTimeRange(startTime: number, endTime: number, options?: QueryOptions): Promise<MetricsRecord[]> {
  ...
  async getAggregated(
    ...
    const result = await this.executeQuery<{ result: number | null }>(
      sql,
      [metricType, metricName, startTime, endTime]
    );
  ...
  async getTrends(
    ...
    const results = await this.executeQuery<{ timestamp: number; value: number }>(
      sql,
      [metricType, metricName, timeframe, limit]
    );

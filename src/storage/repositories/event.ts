...
export class EventRepository extends BaseRepository<EventRecord> {
   ...
   */
  async findByTimeRange(startTime: number, endTime: number, options?: QueryOptions): Promise<EventRecord[]> {
  ...
  async getStatistics(): Promise<{
    ...
    const bySyncStatusResults = await this.executeQuery<{ sync_status: string; count: number }>(bySyncStatusSql);

...
export class ActivityRepository extends BaseRepository<ActivityRecord> {
   ...
   */
  async findByTimeRange(startTime: number, endTime: number, options?: QueryOptions): Promise<ActivityRecord[]> {
   ...
   */
  async findByStageAndAction(stage: string, action: string, options?: QueryOptions): Promise<ActivityRecord[]> {
   ...
   */
  async getStatisticsByStage(): Promise<Record<string, { count: number; actions: Record<string, number> }>> {
    // Get count by stage
    ...
    const actionResults = await this.executeQuery<{ stage: string; action: string; count: number }>(actionSql);
